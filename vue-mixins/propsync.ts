 ...
 */
function getDataName(propName: string): string {
    //注意：映射后名称不能以 $ 或 _ 开头，会被vue认定为内部属性！！
    return "p_" + propName;
}


export default <vuejs.ComponentOption>{
    //修改data，自动生成props对应的data字段
    data: function() {
        var data = {};
        var that: vuejs.Vue = this;
        /** 所有组件定义的props名称数组 */
        var propsKeys: string[] = Object.keys(((<vuejs.ComponentOption>that.$options).props) || {});

        propsKeys.forEach(function(prop: string, i: number) {
            var dataName: string = getDataName(prop);
            var isEnable: boolean = (<vuejs.ComponentOption>that.$options).props[prop][isEnableName];
    ...
    created: function() {
        ...
        var unwatchPropsFnArr: Function[] = [];
        /** 所有 取消data的watch监听函数 的数组 */
        var unwatchDataFnArr: Function[] = [];
        /** 所有组件定义的props名称数组 */
        var propsKeys: string[] = Object.keys(((<vuejs.ComponentOption>that.$options).props) || {});
        ...
        propsKeys.forEach(function(prop: string, i: number) {
            ...
            //监听所有props属性
            var propsFn: Function = that.$watch(prop, function(newVal, oldVal) {
                that[dataName] = newVal;//将组件外变更的prop同步到组件内的p_prop变量中
            }, {});
            unwatchPropsFnArr.push(propsFn);

            //[监听所有属性映射到组件内的变量]
            var dataFn: Function = that.$watch(dataName, function(newVal, oldVal) {
